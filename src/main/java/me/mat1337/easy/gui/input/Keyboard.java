...
public class Keyboard implements KeyListener {
    ...
    private static final Map<Integer, Boolean> registry = new HashMap<>();

    private EasyGui easyGui;

    public Keyboard(EasyGui easyGui) {
    ...
    public void keyPressed(KeyEvent e) {
        ...
        if (!registry.containsKey(keyCode) || !registry.get(keyCode)) {
            easyGui.onKeyPress(e.getKeyChar(), keyCode);
        }
        registry.put(keyCode, true);
    }

    @Override
    public void keyReleased(KeyEvent e) {
        registry.put(e.getKeyCode(), false);
        easyGui.onKeyRelease(e.getKeyChar(), e.getKeyCode());
    }

    public static boolean isKeyDown(int keyCode) {
        return registry.getOrDefault(keyCode, false);
    }
